    ...
    const renderFeaturesEditor = () => {
        ...
        return (
            <>
                    ...
                    {features.map((feature, index) => (
                        <div key={feature.id || index} className="p-4 bg-slate-50 rounded-lg border border-slate-200 space-y-3">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded">{feature.icon}</span>
                            </div>
                            <input
                                type="text"
                                value={feature.title || ''}
                ...
                {/* Stats */}
                <div className="space-y-4">
                    <label className="block text-sm font-bold text-slate-700">Bottom Stats (Small Text with Trophy Icons)</label>
                    {(data.stats || []).map((stat, index) => (
                        <div key={index} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-200">
                            ...
                            <input
                                ...
                                placeholder="e.g., Top 10 Research Universities"
                            />
                        </div>
                    ))}
                    {(!data.stats || data.stats.length === 0) && (
                        <p className="text-sm text-slate-400 italic">No stats configured. Add stats in the database.</p>
                    )}
                </div>
            </>
        );
            ...
            <>
                <div>
                    ...
                    <input
                        type="text"
                        value={data.tagline || ''}
                        onChange={(e) => updateSectionContent('news', 'tagline', e.target.value)}
                        className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm"
                    />
                </div>
                <div>
